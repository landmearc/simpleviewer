...
class ViewerApp(TkinterDnD.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(file_frm, text="※ プレビューエリアへドロップ可",
        ...
        ttk.Label(pdf_ctrl, text="ホイール: ズーム（カーソル中心）　ドラッグ: パン",
                  foreground="gray").pack(anchor="w", pady=(4, 0))
        ...
        self.pdf_canvas.drop_target_register(DND_FILES)
        self.pdf_canvas.dnd_bind("<<Drop>>", self.on_drop_any)
        ...
        self.sim_canvas.drop_target_register(DND_FILES)
        self.sim_canvas.dnd_bind("<<Drop>>", self.on_drop_any)
    ...
    def on_drop_any(self, event):
        """PDF / SIMA どちらでも受け取り、拡張子で振り分ける"""
        for path in norm_paths_from_dnd(event.data):
            if not os.path.isfile(path): continue
            ext = os.path.splitext(path.lower())[1]
            if ext == ".pdf":
                self.load_pdf(path); return
            elif ext in (".sim", ".sima"):
                self.load_sim(path); return
    ...
    def on_pdf_wheel(self, event):
        ...
        self.pdf_zoom_at(event.x, event.y, 1.12 if event.delta > 0 else 1/1.12)
        return "break"
    ...
    def on_pdf_wheel_linux(self, event):
        if not self._pdf_doc: return "break"
        self.pdf_zoom_at(event.x, event.y, 1.12 if event.num == 4 else 1/1.12)
        return "break"
    ...
    def render_pdf_preview(self):
        ...
        self.pdf_canvas.create_text(
            10, 10, text="ホイール: ズーム（カーソル中心）　ドラッグ: パン",
